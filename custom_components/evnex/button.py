...
EVNEX_BUTTONS: tuple[EvnexButtonSensorEntityDescription, ...] = (
    EvnexButtonSensorEntityDescription(
        ...
        press_fn=lambda evnex_api, charger_id, org_id: evnex_api.stop_charge_point(charge_point_id=charger_id, org_id=org_id),
    ),
)
        ...
        for charge_point_obj in charge_points_in_org:
            ...
            if charge_point_detail_v3 and charge_point_detail_v3.connectors:
                for connector_detail_v3 in charge_point_detail_v3.connectors:
                    ...
                    entities.append(
                        EvnexChargerButtonEntity(evnex_api_client,
                            coordinator,
                            entity_description,
                            charger_id,
                            org_id,
...
class EvnexChargerButtonEntity(EvnexChargerEntity, ButtonEntity):
    entity_description: EvnexChargerButtonEntity
    _attr_has_entity_name = True

    def __init__(self, api_client, coordinator, entity_description, charger_id, org_id):
